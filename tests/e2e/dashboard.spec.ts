...
test('clicking a list item opens the detail sheet with correct data', async ({ page }) => {
  ...
  const detailItem = page.locator('#listOverlay.overlay.active .list-item')
    .filter({ hasText: 'Detail Employee' })
    .filter({ hasText: 'Green Bank - Temple Bar' })
    .first()
  await expect(detailItem).toBeVisible()
  await detailItem.locator('.list-open-button').click()
