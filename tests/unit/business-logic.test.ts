...
describe('checkPageAccess', () => {
  describe('admin', () => {
    ...
    it('can access users', () => expect(checkPageAccess('admin', 'users')).toBe(true))
  })
  describe('supervisor', () => {
...
describe('timeAgo', () => {
  ...
  it('returns "Just now" for < 60 seconds', () => {
    ...
    expect(timeAgo(earlier.toISOString())).toBe('Just now')
    expect(timeAgo(now.toISOString())).toBe('Just now')
  })

  it('returns "Xm ago" for < 1 hour', () => {
    const now = new Date()
    const earlier = new Date(now.getTime() - 25 * 60 * 1000)
    expect(timeAgo(earlier.toISOString())).toBe('25m ago')
  })

  it('returns "Xh ago" for < 1 day', () => {
    const now = new Date()
    const earlier = new Date(now.getTime() - 5 * 60 * 60 * 1000)
    expect(timeAgo(earlier.toISOString())).toBe('5h ago')
  })

  it('returns "Xd ago" for < 1 week', () => {
    const now = new Date()
    const earlier = new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000)
    expect(timeAgo(earlier.toISOString())).toBe('3d ago')
  })

  it('returns locale date string for > 1 week', () => {
    const now = new Date()
    const earlier = new Date(now.getTime() - 10 * 24 * 60 * 60 * 1000)
    expect(timeAgo(earlier.toISOString())).toBe(earlier.toLocaleDateString('en-IE'))
  })
})

describe('consecutiveExcellent', () => {
  it('returns 0 when first eval is not excellent', () => {
    expect(consecutiveExcellent([{ overall: 4.0 }, { overall: 5.0 }])).toBe(0)
  })

  it('returns correct streak from most recent', () => {
    expect(consecutiveExcellent([{ overall: 5.0 }, { overall: 4.6 }, { overall: 4.5 }])).toBe(2)
  })
