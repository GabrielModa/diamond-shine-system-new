...
describe('operational insights', () => {
  it('keeps future scheduling gaps in Schedule instead of duplicating them as historical site risk', async () => {
    ...
    expect(response.body.data.summary).not.toHaveProperty('unassignedUpcoming')
    const risk = response.body.data.siteRisks.find((item: { name: string }) => item.name === 'Risk Radar Site')
    expect(risk).toEqual(expect.objectContaining({ name: 'Risk Radar Site', level: 'healthy', score: 0 }))
    expect(risk.reasons).not.toContain('upcoming visit without a team')
  })
})
