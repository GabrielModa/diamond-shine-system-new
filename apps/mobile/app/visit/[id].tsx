...
export default function VisitScreen() {
  ...
  const [declining, setDeclining] = useState(false); const [declineReason, setDeclineReason] = useState('');
  ...
  async function respondToAssignment(status: 'acknowledged' | 'declined') { if (!session || !visit) return; await withAction(async () => {
    const reason = declineReason.trim(); if (status === 'declined' && !reason) throw new Error('Tell operations why you cannot attend.');
    if (!(await online())) throw new Error('Reconnect briefly to send your availability response.');
    ...
    setDeclining(false); setDeclineReason(''); setMessage(status === 'acknowledged' ? 'Visit confirmed. Operations can see your response.' : 'Operations has been notified that you cannot attend.'); await load();
  }); }
