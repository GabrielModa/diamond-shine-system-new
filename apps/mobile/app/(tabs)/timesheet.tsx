...
export default function TimesheetScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMessage('Showing downloaded visit time. Other categories will refresh when connected.');
    } finally {
  ...
  async function start(kind: GenericKind) {
    ...
    await act(async () => {
      ...
      const saveOffline = async () => {
        ...
        await setLocalTimer(timer); setLocalTimerState(timer);
      ...
      try {
        ...
        setMessage(`${label(kind)} timer started.`); await load();
      } catch (cause) {
  ...
  async function stop() {
    ...
    await act(async () => {
      ...
      try {
        ...
        if (localGeneric) await clearLocalTimer(localGeneric.visitId);
        setLocalTimerState(null); setMessage('Timer stopped and added to your timesheet.'); await load();
      } catch (cause) {
        if (!isNetworkApiError(cause)) throw cause;
        await saveOffline();
      }
