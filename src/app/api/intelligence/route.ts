...
export async function GET(request: NextRequest) {
  ...
  const [sites, visits, inspections, actions, stock, supplyRequests, noticeRecipients, feedback] = await Promise.all([
    ...
    prisma.visit.findMany({
      where: { organizationId, scheduledStart: { gte: since, lte: upcomingTo } },
      include: {
  ...
  const siteRisks = sites.map((site) => {
    ...
    const signals = {
      missedOrBlocked: siteVisits.filter((item) => ['missed', 'completion_blocked'].includes(item.status)).length,
      ...
      unassignedUpcoming: siteUpcoming.filter((item) => item.assignments.length < item.requiredWorkers).length,
  ...
  for (const visit of historical) for (const assignment of visit.assignments) {
  ...
  const actionsNow = [
    ...
    ...actions.filter((item) => item.dueAt < now).slice(0, 5).map((item) => ({ priority: item.severity, title: item.title, detail: 'Corrective action is overdue', href: '/feedback' })),
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      summary: {
        ...
        unassignedUpcoming: upcoming.filter((visit) => visit.assignments.length < visit.requiredWorkers).length,
      },
      siteRisks,
      actionsNow,
      team: Array.from(team.values()).map((item) => ({ id: item.id, name: item.name, completedVisits: item.visits.size, minutes: item.minutes, anomalies: item.anomalies, rating: item.ratings.length ? Math.round(item.ratings.reduce((sum, value) => sum + value, 0) / item.ratings.length * 10) / 10 : null })).sort((a, b) => b.completedVisits - a.completedVisits),
    },
  })
}
