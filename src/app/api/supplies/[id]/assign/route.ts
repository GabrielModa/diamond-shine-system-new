...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const supply = await prisma.supplyRequest.findUnique({ where: { id: params.id } })
  ...
  const updated = await prisma.supplyRequest.update({ where: { id: params.id }, data: { assignedTo: assigneeEmail } })
  await logAudit(auth.user.email, 'assign_supply', 'supply', params.id, { assignedTo: assigneeEmail })
