...
export default function TimesheetsWorkspace({ canManage }: { canManage: boolean }) {
  ...
  return <main className="page-shell manager-page">
    ...
    <section className="card timesheet-period-control"><div className="timesheet-period-summary"><div className="timesheet-period-heading"><span className="eyebrow">Review period</span><strong>{from || 'Start'} <span aria-hidden="true">→</span> {to || 'Today'}</strong></div><small>Review and payroll preview always use this same period.</small></div><ListControls query={tab === 'review' ? query : ''} onQueryChange={tab === 'review' ? setQuery : () => undefined} from={from} to={to} onFromChange={setFrom} onToChange={setTo} placeholder="Search employee, site or work…" hasActiveFilters={Boolean((tab === 'review' && query.trim()) || from || to || filter !== 'all')} onClear={() => { setQuery(''); setFrom(''); setTo(''); setFilter('all') }} options={tab === 'review' ? [{ label: 'Review status', value: filter, defaultValue: 'all', choices: [{ value: 'all', label: 'All entries' }, { value: 'needs_review', label: 'Needs review' }, { value: 'approved', label: 'Approved' }], onChange: (value) => setFilter(value as typeof filter) }] : []} /></section>
