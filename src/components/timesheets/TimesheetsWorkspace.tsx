...
export default function TimesheetsWorkspace({ canManage }: { canManage: boolean }) {
  ...
  return <main className="page-shell manager-page">
    ...
    <nav className="manager-tabs" aria-label="Timesheet sections"><button className={tab === 'review' ? 'active' : ''} onClick={() => setTab('review')}>Review time <span>{reviewEntries.length}</span></button>{canManage ? <button className={tab === 'payroll' ? 'active' : ''} onClick={() => setTab('payroll')}>Payroll preview</button> : null}</nav>
