...
export default function EmployeeFeedbackOverview({
  ...
  const filtered = useMemo(
    () => employees.filter((employee) => category === 'all' || employee.category === category),
    [category, employees],
  )
  ...
  return <section className={styles.performancePanel} aria-labelledby="employee-feedback-title">
    <div className={styles.performanceHead}>
      <div className={styles.iconHeading}>
        ...
        <div><span className="eyebrow">Employee ratings</span><h2 id="employee-feedback-title">Service performance by employee</h2><p>Same rating model, now connected directly to the full feedback history below.</p></div>
      </div>
    </div>
    ...
    <div className={styles.performanceMetrics}>
      ...
      <article className={attention ? styles.metricAttention : ''}><OpsIcon name="alert" /><span>Needs attention</span><strong>{attention}</strong><small>Evaluations below 4.0</small></article>
    ...
    <div className={styles.ratingPills} role="group" aria-label="Filter employees by rating">
      {CATEGORIES.map((item) => <button type="button" key={item} className={category === item ? styles.selected : ''} aria-pressed={category === item} onClick={() => setCategory(item)}>
    ...
    <div className={styles.employeeList}>
      ...
      {!filtered.length ? <div className="empty-state compact">No employees match this rating category.</div> : null}
    </div>
  </section>
}
