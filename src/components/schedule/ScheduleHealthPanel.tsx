...
type PausePreview = { target: string; timezone: string; consequence: { canApply: boolean; affectedVisits: number; assignedCleaners: number; plannedLabourHours: number; blockers: Array<{ id: string; site: string }> } }
...
export default function ScheduleHealthPanel({
  ...
  return <section className={styles.panel} aria-label="Schedule intelligence and service continuity">
    ...
    {pauseItem && pauseDraft ? <div className={styles.modalOverlay} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) { setPauseItem(null); setPauseDraft(null); setPreview(null) } }}><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="pause-service-title">
      ...
      {preview ? <div className={styles.impact}><strong>{preview.target}</strong><div className={styles.impactGrid}><div><strong>{preview.consequence.affectedVisits}</strong><span>future visits affected</span></div><div><strong>{preview.consequence.assignedCleaners}</strong><span>cleaners released</span></div><div><strong>{preview.consequence.plannedLabourHours}</strong><span>planned labour hours</span></div></div>{preview.consequence.blockers.map((blocker) => <div className={styles.blocker} key={blocker.id}>Already in progress: {blocker.site}</div>)}</div> : null}
