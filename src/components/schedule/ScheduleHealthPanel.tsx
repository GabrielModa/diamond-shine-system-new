...
export default function ScheduleHealthPanel({
  ...
  return <section className={`${styles.panel} schedule-health-panel`} aria-label="Schedule intelligence and service continuity">
    ...
    {attentionView && (!focus || focus === 'scheduling') && notOnCalendar.length > 0 ? <section className="schedule-pending-work" aria-label="Work not yet on calendar"><header><h2>Schedule attention · {notOnCalendar.length}</h2><p>{notOnCalendar.length > 1 ? 'Showing the first item here. Open details to review the full list without pushing the calendar down.' : 'This client service still needs schedule attention before it is fully covered.'}</p>{notOnCalendar.length > 1 ? <button type="button" className="btn-secondary" onClick={() => openDetails('scheduling')}>Review all {notOnCalendar.length}</button> : null}</header>{notOnCalendar.slice(0, 1).map((item) => <article key={item.id}><div><strong>{item.clientName}{item.siteName ? ` · ${item.siteName}` : ''}</strong><p>{labels[item.state]} · {item.detail}</p>{item.scheduledStart ? <small>{new Date(item.scheduledStart).toLocaleString('en-IE', { timeZone: item.timezone ?? timezone })}</small> : <small>No date set</small>}</div>{item.state === 'expected_not_scheduled' && item.jobId ? <button type="button" className="btn-primary" disabled={busy} onClick={() => void ensureOccurrence(item)}>Create visit</button> : item.servicePlanId ? <button type="button" className="btn-primary" disabled={busy} onClick={() => onOpenServicePlan(item.servicePlanId!)}>Configure service</button> : null}</article>)}</section> : null}
