...
// Diamond System Integrity V10 — schedule lifecycle
...
export default function ScheduleBoard({ canManage, timezone }: { canManage: boolean; timezone: string }) {
  ...
  useEffect(() => {
    const closeForHealth = () => { setShowFindTime(false); setShowFilters(false); window.dispatchEvent(new Event('diamond:close-nav')) }
  ...
  const visibleVisits = useMemo(() => visitsInVisibleRange.filter((visit) => { const activeCount = visit.assignments.filter((assignment) => isActiveAssignment(assignment.status)).length; const historical = ['completed', 'cancelled', 'missed'].includes(visit.status); const statusMatch = statusFilter === 'needs_scheduling' ? !historical && activeCount < visit.requiredWorkers : statusFilter === 'upcoming' ? !historical : statusFilter === 'history' ? historical : visit.status === statusFilter; const teamMatch = teamFilter === 'all' || (teamFilter === 'unassigned' ? activeCount === 0 : visit.assignments.some((assignment) => assignment.user.id === teamFilter && isActiveAssignment(assignment.status))); return statusMatch && teamMatch }), [visitsInVisibleRange, statusFilter, teamFilter])
  ...
  const assignmentState = useCallback((userId: string, startValue: string, endValue: string, ignoredVisitId?: string): AssignmentState | null => {
    ...
    return visit ? { kind: 'busy', label: `Busy · ${timeRange(new Date(visit.scheduledStart), new Date(visit.scheduledEnd), timezone)} at ${visit.site.client.displayName}` } : null
  ...
  return <main className="page-shell schedule-page">
    ...
    <section className="schedule-focus-tabs" aria-label="Schedule focus">
      <button className={statusFilter === 'needs_scheduling' ? 'selected' : ''} onClick={() => setStatusFilter('needs_scheduling')}>Needs scheduling {visitsInVisibleRange.filter((visit) => !['completed', 'cancelled', 'missed'].includes(visit.status) && visit.assignments.filter((assignment) => isActiveAssignment(assignment.status)).length < visit.requiredWorkers).length}</button>
      <button className={statusFilter === 'upcoming' ? 'selected' : ''} onClick={() => setStatusFilter('upcoming')}>Upcoming</button>
      <button className={statusFilter === 'history' ? 'selected' : ''} onClick={() => setStatusFilter('history')}>History</button>
    </section>
