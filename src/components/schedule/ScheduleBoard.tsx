...
export default function ScheduleBoard({ canManage, timezone }: { canManage: boolean; timezone: string }) {
  ...
  }).length, [teamFilter, visitsInVisibleRange, visitHasConflict, focusedEmployeeId])
  ...
  function changeHealthFocus(focus: HealthFocus) { setShowFindTime(false); setShowFilters(false); window.dispatchEvent(new Event('diamond:close-nav')); setHealthFocus(focus); setStatusFilter('attention') }
  ...
  async function saveVisit(status?: 'scheduled' | 'cancelled') {
    ...
    if (status === 'cancelled' && !edit.cancellationReason.trim()) {
      setEditError('Add a cancellation reason before cancelling this visit.')
      return
    }
    if (isTerminalVisit(selected.status)) {
      setEditError('Historical visits are read-only in the operational schedule.')
      return
    }
    ...
    if (status !== 'cancelled') {
      ...
      if (blocked.length) {
        setEditError(blocked.map(({ name, state }) => state.kind === 'busy'
          ? `${name} is already booked ${state.label.replace(/^Busy · /, '')}.`
          : `${name} is ${state.label.toLowerCase()}.`).join(' '))
        return
      }
    }
    ...
    try {
      ...
      setNotice(status === 'cancelled' ? 'Visit cancelled, removed from Operational, team notified and history retained.' : 'Visit updated and the assigned team has been notified.')
      dismissSelected()
      await refresh()
      setHealthRefreshSignal((value) => value + 1)
    } catch (error) {
      setEditError(error instanceof Error ? error.message : 'Could not update this visit.')
    } finally {
      setBusy(false)
    }
  }
