...
export default function ScheduleBoard({ canManage, timezone }: { canManage: boolean; timezone: string }) {
  ...
  }).length, [teamFilter, visitsInVisibleRange])
  ...
  function changeHealthFocus(focus: HealthFocus) { setShowFindTime(false); setShowFilters(false); window.dispatchEvent(new Event('diamond:close-nav')); setHealthFocus(focus); setStatusFilter('upcoming') }
