...
export default function MaterialsWorkspace({ canManage, personalView = false }: { canManage: boolean; personalView?: boolean }) {
...
</article><article className={`card ${styles.queueCard}`}>
  ...
  <div className={styles.queueViewport} aria-label="Supply request queue"><RequestList requests={visibleControlRequests} canManage={canManage} onAdvance={moveRequest} onCancel={(request) => setConfirmTransition({ request, status: 'Cancelled' })} onRepeat={repeatRequest} onOpen={setSelectedRequest} busyId={busyRequest} /></div>
