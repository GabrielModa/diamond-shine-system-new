...
export default function MaterialsWorkspace({ canManage, operationsDeskHref = null }: { canManage: boolean; operationsDeskHref?: string | null }) {
  ...
  return <main className="page-shell materials-shell">
    ...
    {!busy && tab === 'history' ? <section className="card"><div className="section-heading"><div><h2>{canManage ? 'Replenishment requests' : 'My requests'}</h2><p className="muted">{operationsDeskHref ? 'Follow status here; managers process triage and purchasing from Operations desk.' : 'Visible ownership and progress replace chat follow-ups.'}</p></div>{operationsDeskHref ? <Link className="btn-secondary" href={operationsDeskHref}>Open Operations desk</Link> : <span className="section-icon" aria-hidden="true">▤</span>}</div><ListControls query={requestQuery} onQueryChange={setRequestQuery} from={requestFrom} to={requestTo} onFromChange={setRequestFrom} onToChange={setRequestTo} placeholder="Search site, status or material…" onClear={() => { setRequestQuery(''); setRequestFrom(''); setRequestTo('') }} /><RequestList requests={visibleRequests} canManage={canManage && !operationsDeskHref} onAdvance={moveRequest} onRepeat={repeatRequest} busyId={busyRequest} /></section> : null}
  </main>
}
