...
type SiteDetail = Site & { areas: Area[] }
...
export default function OperationsHub({ canManage }: { canManage: boolean }) {
  ...
  const refresh = useCallback(async () => {
    setLoading(true)
    try {
      const [clientRows, contractRows, siteRows, planRows] = await Promise.all([
        api<Client[]>('/api/clients'), api<Contract[]>('/api/contracts'), api<Site[]>('/api/sites'), api<Plan[]>('/api/service-plans'),
      ])
      setClients(clientRows); setContracts(contractRows); setSites(siteRows); setPlans(planRows)
